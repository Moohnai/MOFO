...
def parse_option():
    ...
    # dataset
    parser.add_argument('--video-add-train', type=str, default="../../mnt/welles/scratch/datasets/SSV2/mp4_videos", help='path of motion map data')
    parser.add_argument('--video-add-val', type=str, default="../../mnt/welles/scratch/datasets/SSV2/mp4_videos")
    parser.add_argument('--video-add-test', type=str, default="../../mnt/welles/scratch/datasets/SSV2/mp4_videos")
    
    parser.add_argument('--motion-map-add-train', type=str, default="../../mnt/welles/scratch/datasets/SSV2/mp4_motion_map_videos/train", help='path of motion map data')
    parser.add_argument('--motion-map-add-val', type=str, default="../../mnt/welles/scratch/datasets/SSV2/mp4_motion_map_videos/validation")
    parser.add_argument('--motion-map-add-test', type=str, default="../../mnt/welles/scratch/datasets/SSV2/mp4_motion_map_videos/test")

    parser.add_argument('--dst-BB-path', type=str, default='../../mnt/welles/scratch/datasets/SSV2', help='path to store generated data')
    parser.add_argument('--dataset', type=str, default='SSV2', choices=['Epic_Kitchens','SSV2'], help='dataset to training')
...
def BB_detector (video_add, motion_map_add, json_saved_add):
    flag = 'train' if 'train' in motion_map_add else 'validation' if 'validation' in motion_map_add else 'test'
    SSV_add = '../../mnt/welles/scratch/datasets/SSV2/mp4_videos'
    ...
    for i in vid:
        id = i['id']
        path = os.path.join(SSV_add, id+'.mp4')
    ...
    videos = []
    for i in vid_id:
        videos.append(os.path.join(SSV_add, i+'.mp4'))
    print('begin')
    ...
    def json_creator (video_add, video_name, motion_map_add, json_saved_add):
        ...
        out_path = os.path.join(json_saved_add, f"BB_{flag}", f"EPIC_100_unsupervised_BB_mm_{video_name.split('.')[0]}.json")
...
print("train BB_detector is done") 
BB_detector(args.video_add_val, args.motion_map_add_val, args.dst_BB_path)
print("val BB_detector is done")
BB_detector(args.video_add_test, args.motion_map_add_test, args.dst_BB_path)
print("test BB_detector is done")
...
for file in json_files:
    with open(file, "r", encoding="utf-8") as f:
        ...
        # x1 = json_file[list(json_file.keys())[0]][0]['labels'][0]['box2d']['x1']
        # y1 = json_file[list(json_file.keys())[0]][0]['labels'][0]['box2d']['y1']
        # x2 = json_file[list(json_file.keys())[0]][0]['labels'][0]['box2d']['x2']
        # y2 = json_file[list(json_file.keys())[0]][0]['labels'][0]['box2d']['y2']
        # video_name = list(json_file.keys())[0]
        # video_path = os.path.join(args.video_add_train, video_name)
        # video = decord.VideoReader(video_path)
        # video_len = len(video)
        # video_array = video.get_batch(range(video_len)).asnumpy()
        # w, h, _ = video_array[0].shape
        # if ((x2-x1) * (y2-y1) < 0.01*w*h):
        #     print(f"remove {video_name} from BB_train")
        #     os.remove(file)
...
with open(os.path.join(args.dst_BB_path, "Unsupervised_BB_SSV2_train.json"), "w+", encoding="utf-8") as f:
...
for file in json_files:
    try:
        with open(file, "r", encoding="utf-8") as f:
            ...
            # w, h, _ = video_array[0].shape
            # if ((x2-x1) * (y2-y1) < 0.01*w*h):
            #     print(f"remove {video_name} from BB_validation")
            #     os.remove(file)
...
# join all json files in one json file
print("join all json files in one json file")
with open(os.path.join(args.dst_BB_path, "Unsupervised_BB_SSV2_validation.json"), "w+", encoding="utf-8") as f:
    f.write(orjson.dumps(json_list).decode("utf-8"))

print("validation is done")

# read all json files
print("read all json files in BB_test")
json_files = glob(os.path.join(args.dst_BB_path, "BB_test", "*.json"))
json_list = {}
for file in json_files:
    with open(file, "r", encoding="utf-8") as f:
        json_file = orjson.loads(f.read())
        ...
        # w, h, _ = video_array[0].shape
        # if ((x2-x1) * (y2-y1) < 0.01*w*h):
        #     print(f"remove {video_name} from BB_test")
        #     os.remove(file)
        ...
        json_list.update(json_file)
print("join all json files in one json file")
with open(os.path.join(args.dst_BB_path, "Unsupervised_BB_SSV2_test.json"), "w+", encoding="utf-8") as f:
    f.write(orjson.dumps(json_list).decode("utf-8"))
print("test is done")
